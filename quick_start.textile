...
Here you'll find everything you need to install and to use Rbjs. Knowledge of Rails and Javascript is assumed.
...
Similar to prototype-rails, there are two ways to write remote javascript: Directly by using the _render_ method within the controller, or within an _.rbjs_ template. The second way is cleaner, since it keeps your view code outside of the controller, while the first way is much more pragmatic for simple and small snippets.

p(tip). The main idea behind Rbjs is to write ruby code the same way you would write javascript code and to keep the logic away from the browser. Within an .rbjs-file, all method calls and their arguments are directly translated to javascript. This is true for any method and argument, including arrays, hashes, rendered partials, ruby blocks and lambdas, and anything that has a _to_json_ or a _to_s_ method.


h3. Using the render method

Within an controller action, which is requested via xhr by your browser, you can call _render :js_ with a block containing the following rbjs code:
...
Of course, working with ruby means that array and hash access also count as methods. This lets us use them within Rbjs, too:
...
p(tip). You can access instance variables, view helper methods, and the local scope inside of Rbjs. As you can see, the calls to _current_user_, _message_, and _@user_id_ are not translated to javascript.
...
As you have seen above, any arguments that get passed to javascript functions are being translated too. But this does not only work for simple arguments, like strings and numbers. You can work with any kind of object.
...
Symbols are converted to strings, while there is no change on strings and numbers. Using this behavior, we can use Rbjs to easily update rendered partials:
...
p(tip). Whenever you pass an argument to a javascript method, Rbjs will invoke its _to_json_ method.

You can also pass nested Rbjs as arguments. This functionality can also be arbitrarily nested.
...
Use caution when working with local ruby variables to avoid getting confused. The following Rbjs example will not render what you expect.
...
p(note). You need to be careful when using operators on javascript elements, eg. for string concatenation. You can read more about using operators down below.
...
p(tip). This limitation is addressed with the release of ruby 2.0 and refinements. For now, you can work around this limitation by wrapping the expression like so: window.alert(Rbjs::Expression.new("'Hello, '+ name")).
